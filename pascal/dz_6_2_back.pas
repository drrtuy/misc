...
//Проход по бОльшему массиву. Каждая итерация содержит проход по меньшему массиву.
  ...
  //Проходим по бОльшему массиву и добавляем в результат все не совпавшие элементы.
  for i := 0 to bigLength-1 do
  begin
     if bigArr^[it].Seen <> False then
        break;

     ArrSymDiff.ArrPntr^[ite].Value := bigArr^[it].Value;
     ite += 1;
     ArrSymDiff.Length^ += 1;
  end;
